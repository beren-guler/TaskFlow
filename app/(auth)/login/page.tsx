...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    router.push('/boards'); router.refresh()
  }
    ...
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: 'radial-gradient(ellipse at 50% 0%, rgba(249,115,22,.08) 0%, var(--bg) 60%)' }}>
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-extrabold text-[var(--text)]">TaskFlow'a Hoş Geldin</h1>
          <p className="text-sm text-[var(--text-2)] mt-1">Hesabına giriş yap</p>
        </div>
        ...
        <div className="glass rounded-2xl p-6 space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-[var(--text-3)]">veya</span>
            <div className="h-px flex-1 bg-[var(--border)]"/>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="relative">
              <Input label="Şifre" type={showPw ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" required className="pr-10" />
            ...
            <div className="text-right">
              <Link href="/forgot-password" className="text-xs text-accent hover:underline">Şifremi unuttum</Link>
            </div>
        ...
        <p className="text-center text-sm text-[var(--text-2)] mt-6">
          Hesabın yok mu?{' '}
          <Link href="/register" className="text-accent font-semibold hover:underline">Kayıt ol</Link>
        </p>
      </div>
    </div>
  )
